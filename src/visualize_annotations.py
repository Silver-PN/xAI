...
def visualize_annotations(label_ids,
                          bboxes,
                          dataset,
                          images,
                          visualizations,
                          target_width,
                          image_filter,
                          is_saved_only_label=True,
                          is_saved_only_bboxes=True,
                          is_saved_both=True):
        ...
        if os.path.exists(os.path.join(bboxes, dataset, bboxes_file)) and image_filter in target:
            ...
            if os.path.exists(image_path):
                ...
                if os.path.exists(label_ids_path):
                    label_ids = cv2.imread(label_ids_path, 0)
                    label_ids = cv2.resize(ids2colors(label_ids, labels), target_size, cv2.INTER_LINEAR)
                else:
                    label_ids = np.zeros((target_size[1], target_size[0], 3), dtype=np.uint8)
                filename=image_path.split('\\')[-1]                           
                
                  
                # Write image with only label_id
                if is_saved_only_label:
                    only_label_dir = os.path.join(visualizations_dir, 'only_label')
                    os.makedirs(only_label_dir, exist_ok=True)                  
                    cv2.imwrite(os.path.join(only_label_dir, filename), label_ids)
                
                # Write image with only bboxes
                if is_saved_only_bboxes:
                    only_bboxes_dir = os.path.join(visualizations_dir, 'only_bboxes')
                    os.makedirs(only_bboxes_dir, exist_ok=True)
                    cv2.imwrite(os.path.join(only_bboxes_dir, filename), image)
                ...
                # # Write image with only both label_id and bboxes
                both_dir = os.path.join(visualizations_dir, 'both')
                os.makedirs(both_dir, exist_ok=True)
                
                if is_saved_both:
                    cv2.imwrite(os.path.join(both_dir, filename), image)
            else:
